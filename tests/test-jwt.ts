    ...
    try {
        ...
        try {
            ...
            if (error instanceof TokenExpiredError) {
                console.log('✅ Successfully caught expired token error:', error.message);
            }
        }

        // 5. Decode expired token (should work despite expiration)
        const decodedExpired = await JWT.decode(expiredToken);
        console.log('\n📖 Decoded Expired Token (decode still works):', decodedExpired);
        ...
        try {
            ...
            if (error instanceof BadTokenError) {
                console.log('✅ Successfully caught invalid token error:', error.message);
            }
        }
